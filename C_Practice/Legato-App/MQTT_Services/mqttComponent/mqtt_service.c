...
static void mqttService_IncomingMessageHandler(void *reportPtr, void *incomingMessageHandler)
{
  ...
  LE_DEBUG("topic('%s') key('%s') value('%s') ts('%s')",
           eventDataPtr->topicName,
...
static mqttClient_t *mqttService_getClient(void)
{
  return &mqttClient;
};
...
int32_t mqtt_GetConnectionState(void)
{
