...
#include "mqttSer_interface.h"
#include "mqtt_messages.h"
#include "mqttSer_service.h"
...
typedef struct
{
    ...
    mqttSer_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(mqttSer_ClientThreadData,
...
)
{
    ...
    le_result_t result = ifgen_mqtt_OpenSession(sessionRef, isBlocking);
    if (result != LE_OK)
    {
...
)
{
    ...
    else
    {
        ...
        LE_FATAL_IF(clientThreadPtr==NULL,
                    "mqttSer_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    ifgen_mqtt_InitCommonData();

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_InitStaticPool(mqttSer_ClientThreadData,
                                                  LE_CDATA_COMPONENT_COUNT,
                                                  sizeof(_ClientThreadData_t));
...
)
{
    ...
    LE_FATAL("Component for mqttSer disconnected\n");
}
 ...
 */
//--------------------------------------------------------------------------------------------------
void mqttSer_SetServerDisconnectHandler
(
...
{
    return ifgen_mqtt_GetConnectionState(
        GetCurrentSessionRef()
    );
}
...
{
    return ifgen_mqtt_AddSessionStateHandler(
        GetCurrentSessionRef(),
        handlerPtr,
        contextPtr
    );
...
{
     ifgen_mqtt_RemoveSessionStateHandler(
        GetCurrentSessionRef(),
        handlerRef
    );
}
...
{
    return ifgen_mqtt_AddIncomingMessageHandler(
        GetCurrentSessionRef(),
        handlerPtr,
        contextPtr
    );
...
{
     ifgen_mqtt_RemoveIncomingMessageHandler(
        GetCurrentSessionRef(),
        handlerRef
    );
}
