 ...
 */
//--------------------------------------------------------------------------------------------------
LE_MEM_DEFINE_STATIC_POOL(mqtt_ClientData,
 ...
 */
//--------------------------------------------------------------------------------------------------
LE_REF_DEFINE_STATIC_MAP(mqtt_ClientHandlers,
    LE_MEM_BLOCKS(mqtt_ClientData, HIGH_CLIENT_DATA_COUNT));
 ...
 *
 * @warning Use mqtt_ClientMutex, defined below, to protect accesses to this data.
 */
//--------------------------------------------------------------------------------------------------
 ...
 */
//--------------------------------------------------------------------------------------------------
__attribute__((unused)) static pthread_mutex_t mqtt_ClientMutex = PTHREAD_MUTEX_INITIALIZER;   // POSIX "Fast" mutex.

/// Locks the mutex.
#define _LOCK    LE_ASSERT(pthread_mutex_lock(&mqtt_ClientMutex) == 0);

/// Unlocks the mutex.
#define _UNLOCK  LE_ASSERT(pthread_mutex_unlock(&mqtt_ClientMutex) == 0);
...
)
{
    ...
    _LOCK;
    {
        if (!_ClientDataPool)
        {
            ...
            _ClientDataPool = le_mem_InitStaticPool(mqtt_ClientData,
                                                    HIGH_CLIENT_DATA_COUNT,
                                                    sizeof(_ClientData_t));
        }
        ...
        if (!_HandlerRefMap)
        {
            ...
            _HandlerRefMap = le_ref_InitStaticMap(mqtt_ClientHandlers,
                                                  LE_MEM_BLOCKS(mqtt_ClientData,
                                                                HIGH_CLIENT_DATA_COUNT));
        }
...
)
{
    ...
    if ( strnlen(brokerUrl, 256) > 256 )
    {
        LE_FATAL("strnlen(brokerUrl, 256) > 256");
    }
...
)
{
    ...
    // Range check values, if appropriate
    if ( strnlen(password, 128) > 128 )
    {
        LE_FATAL("strnlen(password, 128) > 128");
    }
...
)
{
    ...
    // Range check values, if appropriate
    if ( strnlen(key, 128) > 128 )
    {
        LE_FATAL("strnlen(key, 128) > 128");
    }
    if ( strnlen(value, 128) > 128 )
    {
        LE_FATAL("strnlen(value, 128) > 128");
    }
...
)
{
    ...
    // Range check values, if appropriate
    if ( strnlen(json, 2048) > 2048 )
    {
        LE_FATAL("strnlen(json, 2048) > 2048");
    }
...
static void _Handle_ifgen_mqtt_AddSessionStateHandler
(
    void* _reportPtr,
...
)
{
    ...
    int32_t connectErrorCode = 0;
    int32_t subErrorCode = 0;

        // Pull out additional data from the client data pointer
        mqtt_SessionStateHandlerFunc_t _handlerRef_ifgen_mqtt_AddSessionStateHandler =
            (mqtt_SessionStateHandlerFunc_t)_clientDataPtr->handlerPtr;
                                               ...
                                               &subErrorCode ))
    {
        ...
        if ( _handlerRef_ifgen_mqtt_AddSessionStateHandler != NULL )
        {
            _handlerRef_ifgen_mqtt_AddSessionStateHandler(isConnected, connectErrorCode, subErrorCode, contextPtr );
        }
        else
        {
...
)
{
    ...
    _UNLOCK
    handlerRef = (mqtt_SessionStateHandlerRef_t)
...
// This function parses the message buffer received from the server, and then calls the user
// registered handler, which is stored in a client data object.
static void _Handle_ifgen_mqtt_AddIncomingMessageHandler
(
    void* _reportPtr,
    void* _sentClientDataPtr
)
{
    ...
    { // This block scope is need to prevent "transfer of control bypasses initialization" error.
        ...
        // Declare temporaries for input parameters
    char topicName[129] = {0};
    char key[129] = {0};
    char value[129] = {0};
    char timestamp[17] = {0};

        // Pull out additional data from the client data pointer
        mqtt_IncomingMessageHandlerFunc_t _handlerRef_ifgen_mqtt_AddIncomingMessageHandler =
            (mqtt_IncomingMessageHandlerFunc_t)_clientDataPtr->handlerPtr;
        void* contextPtr = _clientDataPtr->contextPtr;
                               ...
                               16 ))
    {
        ...
        // Call the registered handler
        if ( _handlerRef_ifgen_mqtt_AddIncomingMessageHandler != NULL )
        {
            _handlerRef_ifgen_mqtt_AddIncomingMessageHandler(topicName, key, value, timestamp, contextPtr );
        }
        else
        {
            LE_FATAL("Error in client data: no registered handler");
...
)
{
    ...
    le_ref_DeleteRef(_HandlerRefMap, handlerRef);
    _UNLOCK
    handlerRef = (mqtt_IncomingMessageHandlerRef_t)
         clientDataPtr->handlerRef;
    le_mem_Release(clientDataPtr);
    LE_ASSERT(le_pack_PackReference( &_msgBufPtr,
                                     handlerRef ));
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_mqtt_AddSessionStateHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_mqtt_AddSessionStateHandler, msgRef, clientDataPtr);
            break;
        case _MSGID_mqtt_AddIncomingMessageHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ifgen_mqtt_AddIncomingMessageHandler, msgRef, clientDataPtr);
            break;
